import java.sql.*;
import java.util.Scanner;

//File ->Project Structure
// -> Module -> Dependencies -> Click the + buttom
// add the Jar or directory
public class InsertTest {

    public static void main(String[] args) {

        try {
            //1. load the driver
            DriverManager.registerDriver(new com.mysql.cj.jdbc.Driver());
            System.out.println("driver registered");

            String url = "jdbc:mysql://localhost:3306/mysql";
            String user = "root";
            String password = "root@123";


            Connection conn = DriverManager.getConnection(url,user,password);
            System.out.println("connection established "+conn);

            PreparedStatement pst = conn.prepareStatement("INSERT INTO DEPT VALUES (?,?,?)");
            System.out.println("prepared statement established "+pst);

            Scanner sc = new Scanner(System.in);
            Scanner sc2 = new Scanner(System.in);
            Scanner sc3 = new Scanner(System.in);

            System.out.println("enter the new dept number of the record");
            int x = Integer.parseInt(sc.nextLine());
            System.out.println("enter the new dept name of the record");
            String y = sc2.nextLine();
            System.out.println("enter the new dept location of the record");
            String z = sc3.nextLine();

            pst.setInt(1,x);
            pst.setString(2,y);
            pst.setString(3,z);

            int rows = pst.executeUpdate();
            System.out.println(rows+ " rows inserted ");


            pst.close();
            conn.close();
            System.out.println("db resources closed");
        } catch (SQLException e) {
            throw new RuntimeException(e);
        }

    }
}
